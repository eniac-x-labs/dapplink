...
LEDGER_ENABLED ?= true
BINDIR ?= $(GOPATH)/bin
BITOS_BINARY = dapplinkd
BITOS_DIR = dapplink
BUILDDIR ?= $(CURDIR)/build
SIMAPP = ./app
HTTPS_GIT := https://github.com/eniac-x-labs/dapplink.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=dapplink \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(BITOS_BINARY) \
...
build-docker:
	...
	$(DOCKER) rm dapplink || true
	# create a new container from the latest image
	$(DOCKER) create --name dapplink -t -i ${DOCKER_IMAGE}:latest dapplink
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp dapplink:/usr/bin/dapplinkd ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/eniac-x-labs/dapplink/types"
	godoc -http=:6060
...
test-sim-custom-genesis-fast:
	...
	@echo "By default, ${HOME}/.$(BITOS_DIR)/config/genesis.json will be used."
	@go test -mod=readonly $(SIMAPP) -run TestFullAppSimulation -Genesis=${HOME}/.$(BITOS_DIR)/config/genesis.json \
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.$(BITOS_DIR)/config/genesis.json will be used."
	@$(BINDIR)/runsim -Genesis=${HOME}/.$(BITOS_DIR)/config/genesis.json -SimAppPkg=$(SIMAPP) -ExitOnFail 400 5 TestFullAppSimulation
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/docs/statik/statik.go" -not -name '*.pb.go' | xargs goimports -w -local github.com/eniac-x-labs/dapplink
.PHONY: format
...
ifeq ($(OS),Windows_NT)
	...
	IF not exist "build/node0/$(BITOS_BINARY)/config/genesis.json" docker run --rm -v $(CURDIR)/build\dapplink\Z dapplinkd/node "./dapplinkd testnet --v 4 -o /dapplink --keyring-backend=test --ip-addresses dapplinkdnode0,dapplinkdnode1,dapplinkdnode2,dapplinkdnode3"
	docker-compose up -d
else
	mkdir -p localnet-setup
	@$(MAKE) localnet-build

	if ! [ -f localnet-setup/node0/$(BITOS_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/localnet-setup:/dapplink:Z dapplinkd/node "./dapplinkd testnet --v 4 -o /dapplink --keyring-backend=test --ip-addresses dapplinkdnode0,dapplinkdnode1,dapplinkdnode2,dapplinkdnode3"; fi
	docker-compose up -d
endif
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\localnet-setup\node0\dapplinkd:dapplink\Z dapplinkd/node "./dapplinkd unsafe-reset-all --home=/dapplink"
	@docker run --rm -v $(CURDIR)\localnet-setup\node1\dapplinkd:dapplink\Z dapplinkd/node "./dapplinkd unsafe-reset-all --home=/dapplink"
	@docker run --rm -v $(CURDIR)\localnet-setup\node2\dapplinkd:dapplink\Z dapplinkd/node "./dapplinkd unsafe-reset-all --home=/dapplink"
	@docker run --rm -v $(CURDIR)\localnet-setup\node3\dapplinkd:dapplink\Z dapplinkd/node "./dapplinkd unsafe-reset-all --home=/dapplink"
else
	@docker run --rm -v $(CURDIR)/localnet-setup/node0/dapplinkd:/dapplink:Z dapplinkd/node "./dapplinkd unsafe-reset-all --home=/dapplink"
	@docker run --rm -v $(CURDIR)/localnet-setup/node1/dapplinkd:/dapplink:Z dapplinkd/node "./dapplinkd unsafe-reset-all --home=/dapplink"
	@docker run --rm -v $(CURDIR)/localnet-setup/node2/dapplinkd:/dapplink:Z dapplinkd/node "./dapplinkd unsafe-reset-all --home=/dapplink"
	@docker run --rm -v $(CURDIR)/localnet-setup/node3/dapplinkd:/dapplink:Z dapplinkd/node "./dapplinkd unsafe-reset-all --home=/dapplink"
endif
...
.PHONY: build-docker-local-dapplink localnet-start localnet-stop
