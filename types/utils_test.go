...
func init() {
	cfg := sdk.GetConfig()
	cfg.SetBech32PrefixForAccount("dapplink", "dapplinkpub")
}
...
func TestGetDappLinkAddressFromBech32(t *testing.T) {
	testCases := []struct {
		name       string
		address    string
		expAddress string
		...
		{
			"dapplink address",
			"dapplink1qql8ag4cluz6r4dz28p3w00dnc9w8ueuafmxps",
			"dapplink1qql8ag4cluz6r4dz28p3w00dnc9w8ueuafmxps",
			false,
		},
		{
	...
	for _, tc := range testCases {
		addr, err := GetDappLinkAddressFromBech32(tc.address)
		if tc.expError {
