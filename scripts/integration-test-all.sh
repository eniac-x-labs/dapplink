...
## default port prefixes for dapplinkd
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    "$PWD"/build/dapplinkd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/dapplinkd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/dapplinkd add-genesis-account \
    "$("$PWD"/build/dapplinkd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000abit,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/dapplinkd gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/dapplinkd collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/dapplinkd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/dapplinkd start --pruning=nothing --rpc.unsafe \
    ...
    BITOS_PID=$!
    echo "started dapplink node, pid=$BITOS_PID"
    # add PID to array
    arr+=("$BITOS_PID")
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test dapplink node $HOST_RPC ..."
...
stop_func() {
    BITOS_PID=$i
    echo "shutting down node, pid=$BITOS_PID ..."

    # Shutdown dapplink node
    kill -9 "$BITOS_PID"
    wait "$BITOS_PID"
