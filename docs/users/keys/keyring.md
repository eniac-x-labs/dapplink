...
```bash
dapplinkd keys add mykey

# Put the generated address in a variable for later use.
MY_VALIDATOR_ADDRESS=$(dapplinkd keys show mykey -a)
```
...
::: warning
**NOTE**: Cosmos `secp256k1` keys are not supported on dapplink due to compatibility issues with Ethereum transactions.
:::
...
yes $KEYPASSWD | dapplinkd keys add me
yes $KEYPASSWD | dapplinkd keys show me
# start dapplinkd with keyring-backend flag
dapplinkd --keyring-backend=file start
```
