...
- [Install dapplink](./../quickstart/installation.md) {prereq}

## 1. Upgrade the dapplink version

Before upgrading the dapplink version. Stop your instance of `dapplinkd` using `Ctrl/Cmd+C`.

Next, upgrade the software to the desired release version. Check the dapplink [releases page](https://github.com/dapplink/dapplink/releases) for details on each release.
...
Verify that you've successfully installed dapplink on your system by using the `version` command:

```bash
$ dapplinkd version --long

name: dapplink
server_name: dapplinkd
version: 3.0.0
...
::: tip
If the software version does not match, then please check your `$PATH` to ensure the correct `dapplinkd` is running.
:::
...
```bash
rm $HOME/.dapplinkd/config/addrbook.json
