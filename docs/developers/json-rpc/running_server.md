...
Learn how to run and setup the JSON-RPC server on dapplink. {synopsis}
...
```bash
dapplinkd start --json-rpc.enable
```
...
```bash
dapplinkd start --json-rpc.api eth,txpool,personal,net,debug,web3,miner
```
...
```bash
# set gas cap to 85M
dapplinkd start --json-rpc.gas-cap 85000000000

# set gas cap to infinite (=0)
dapplinkd start --json-rpc.gas-cap 0
```
