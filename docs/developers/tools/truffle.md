...
Learn how to deploy a simple Solidity-based smart contract to dapplink using the Truffle environment {synopsis}
...
This will allow your contract to connect to your dapplink local node.
...
You should see incoming deployment logs in the dapplink daemon Terminal tab for each transaction (one to deploy `Migrations.sol` and the other to deploy `Counter.sol`).
...
Now, you can run the Truffle tests using the dapplink node using the `test` command:
